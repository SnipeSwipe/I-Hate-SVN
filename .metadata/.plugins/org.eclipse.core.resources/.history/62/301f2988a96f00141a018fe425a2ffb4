...
public class Midfielder extends Player {
	...
	public Midfielder(PlayPanel panel, int starty, int startx) {
		this.panel = panel;
		isMoving = true;
		size = 20;
		speed = 50;
	...
	public Midfielder(PlayPanel panel, int starty, int startx, int end) {
		this.panel = panel;
		isMoving = true;
		size = 20;
		speed = 50;
	...
	public void run() {
		while (isMoving) {
			...
			// Work in Progress
			if (newy + size > (panel.getHeight() - end) || newy < (0 + end)) {
				dy = -dy;
			}
